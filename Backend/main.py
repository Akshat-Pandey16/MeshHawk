...
def execute_script(duration: int):
    script_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "scripts/first.sh")
    try:
...
def execute_attack(bssid:str,channel: int):
    script_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "scripts/second.sh")
    print(script_path)
    print(type(script_path))
    try:
        subprocess.Popen(["gnome-terminal", "--", "bash", "-c", f"cd scripts/ && {script_path} {bssid} {channel}"])
    except subprocess.CalledProcessError:
        raise HTTPException(status_code=500, detail="Error executing the script")
